...
from envs.overcooked.overcooked_ai.overcooked_ai_py.utils import pos_distance, load_from_json
from envs.overcooked.overcooked_ai.overcooked_ai_py.data.layouts import read_layout_dict
from envs.overcooked.overcooked_ai.overcooked_ai_py.mdp.actions import Action, Direction
...
class OvercookedGridworld(object):
    ...
    def __init__(self, terrain, start_player_positions, start_order_list=None, cook_time=20, num_items_for_soup=3, delivery_reward=20, rew_shaping_params=None, layout_name="unnamed_layout", **kwargs):
        """
    ...
    def get_state_transition(self, state, joint_action):
        ...
        if np.random.rand() < 0.01:
            ...
            joint_action = [actions[np.random.choice(len(actions))] for actions in valid_actions]
