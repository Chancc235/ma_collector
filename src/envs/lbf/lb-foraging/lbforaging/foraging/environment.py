...
class ForagingEnv(Env):
    ...
    Observation = namedtuple(
        "Observation",
        ["field", "actions", "players", "game_over", "sight", "current_step", "abs_position", "score"],
    )
    ...
    def get_observation_space(self):
        ...
        if not self._grid_observation:
            ...
            min_obs = [0, 0, 0] + [-1, -1, 0] * max_food + [0, 0, 1] * len(self.players)
            max_obs = [field_x, field_y, 1] + [field_x, field_y, max_food_level] * max_food + [
                field_x,
                field_y,
    ...
    def spawn_food(self, max_food, max_level):
        if self.fixed_env:
            assert max_food == 4  # please use grid with 4 food
            # four directions
            self.field[1, self.cols // 2] = max_level
            self.field[self.rows-2, self.cols // 2] = max_level
            self.field[self.rows // 2, 1] = max_level
            self.field[self.rows // 2, self.cols-2] = max_level
    ...
    def getvalid_actions(self) -> list:
        return list(product(*[self.valid_actions[player] for player in self.players]))
    ...
    def _make_gym_obs(self, sight):
        def make_obs_array(observation):
            ...
            # ! extra observation of [x,y,r] of player itself, append at last
            obs[-3] = observation.abs_position[0]
            obs[-2] = observation.abs_position[1]
    ...
    def step(self, actions):
        ...
        actions = [
            Action(a) if Action(a) in self.valid_actions[p] else Action.NONE
            for p, a in zip(self.players, actions)
        ]

        # check if actions are valid
        for i, (player, action) in enumerate(zip(self.players, actions)):
            if action not in self.valid_actions[player]:
                self.logger.info(
    ...
    def _init_render(self):
        ...
        self.viewer = Viewer((self.rows, self.cols), self.remote)
    ...
    def get_state(self):
        nobs, _, _, _ = self._make_gym_obs(sight=self.field_size[0])
        return nobs[0]
