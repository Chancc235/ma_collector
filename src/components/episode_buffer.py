...
class EpisodeBatch:
    ...
    def update(self, data, bs=slice(None), ts=slice(None), mark_filled=True):
        ...
        for k, v in data.items():
            ...
            # ! modified, speed up tensor creating
            if type(v) == list:
                v = np.array(v)
                v = th.tensor(v, dtype=dtype, device=self.device)
            elif type(v) == th.Tensor and v.dtype == dtype:
                v.to(self.device)
            else:
                v = th.tensor(v, dtype=dtype, device=self.device)
...
class MetaReplayBuffer(ReplayBuffer):
    def __init__(self, scheme, groups, buffer_size, max_seq_length, preprocess=None, device="cpu"):
        super().__init__(scheme, groups, buffer_size,
    ...
    def fetch_newest_batch(self, batch_size):
        ...
        if batch_size > self.episodes_in_buffer:
            raise Exception(f"-->> request size larger than current size")
        if self.buffer_index >= batch_size:
            ...
            del scheme["filled"]
            ret = ReplayBuffer(scheme, self.groups, batch_size, self.max_seq_length, self.preprocess, self.device)
            ret.insert_episode_batch(self[self.buffer_index-batch_size:])
            ret.insert_episode_batch(self[:self.buffer_index])
            return ret

    def select(self, ep_batch, agent_ids):
        """
        return selected data corresponding to agent_ids
        """
        selected_batch = copy.deepcopy(ep_batch)
        for k, v in ep_batch.data.transition_data.items():
            group = ep_batch.scheme[k].get("group", None)
            if group:
                selected_batch.data.transition_data[k] = v[:, :, agent_ids, :]
        for k, v in ep_batch.data.episode_data.items():
            group = ep_batch.scheme[k].get("group", None)
            if group:
                selected_batch.data.episode_data[k] = v[:, :, agent_ids, :]
